...
pub async fn dispatch(
    ...
    match req.method.as_str() {
        ...
        _ => JsonRpcResponse::err(req.id, RPC_METHOD_NOT_FOUND,
...
async fn handle_agents_get(id: Option<Value>, params: Option<Value>, state: &AppState) -> JsonRpcResponse {
    let agent_id = params
        .as_ref()
    ...
    if agent_id != "default" {
        ...
        match state.federated_agent_registry.get_agent(&agent_id).await {
            ...
            Ok(None) => {
                return JsonRpcResponse::err(id, ACP_AGENT_NOT_FOUND,
...
async fn handle_sessions_create(
    id: Option<Value>, params: Option<Value>,
    _state: &AppState, sessions: &AcpSessionStore,
) -> JsonRpcResponse {
    let agent_id = params
        .as_ref()
        .and_then(|p| p.get("agent_id"))
...
async fn handle_sessions_get(
    id: Option<Value>, params: Option<Value>, sessions: &AcpSessionStore,
) -> JsonRpcResponse {
    let session_id = match params.as_ref().and_then(|p| p.get("session_id")).and_then(Value::as_str) {
        Some(s) => s.to_string(),
    ...
    match sessions.get(&session_id).await {
        ...
        None => JsonRpcResponse::err(id, ACP_SESSION_NOT_FOUND, format!("Session '{session_id}' not found")),
    }
}
...
async fn handle_runs_create(
    id: Option<Value>, params: Option<Value>,
    state: &AppState, sessions: &AcpSessionStore,
) -> JsonRpcResponse {
...
async fn handle_runs_get(
    id: Option<Value>, params: Option<Value>, state: &AppState,
) -> JsonRpcResponse {
    let run_id = match params.as_ref().and_then(|p| p.get("run_id")).and_then(Value::as_str) {
        Some(r) => r.to_string(),
