...
| CH-18 | Zero-code bossfang seam: `/v1/chat/completions` OpenAI-compat round-trip with no UAR extension fields | `zero_code_seam_v1_chat_completions_is_openai_compatible` (`tests/integration/live/librefang_seam_cases.rs`) | Real server + stub LLM, recorded backend. Proves the exact path a bossfang `provider_urls` entry would target. See `docs/librefang-integration.md`. |
| CH-21 | AG-UI official event vocabulary (`RUN_STARTED`/`TEXT_MESSAGE_CONTENT`/`RUN_FINISHED`) via `stream_mode: "agui_spec"` | `agui_spec_mode_emits_official_event_vocabulary` (`tests/integration/live/librefang_seam_cases.rs`) | Finishes CH-21: `to_agui_spec_event` existed but was never reachable via any `stream_mode` until this pass wired the `agui_spec` mode into `src/server.rs`'s streaming loop. |
| CH-20 (`uar-spec-v2-and-polish` phase) | Concurrent-agent load: 50 concurrent `/api/chat/completion` requests against one real server, measuring latency/throughput and asserting zero failures | `concurrent_agents_load_test` (`tests/integration/live/load_test.rs`) | Soak/smoke test, not a strict perf-regression gate (CI hardware varies too much for a hard SLA) — prints real latency percentiles (p50/p95/max) and throughput for human review; the hard assertion is zero request failures under load. |
