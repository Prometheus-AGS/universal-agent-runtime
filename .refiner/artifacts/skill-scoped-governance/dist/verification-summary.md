...
- Compatibility: PASS after second correction. The pre-existing replacement assertions pass, and a pending legacy allowlist now affects skills loaded later through the matcher while explicit conversation and durable agent records retain precedence.
...
Uncomfortable result: the independent reviews correctly rejected the
same-handle “restart” proof and found that the durable model had silently
dropped the legacy binding API's unknown IDs. The first compatibility repair
fixed GET but not matching; that second gap required another critic pass.
