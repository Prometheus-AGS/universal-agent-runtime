...
### Remaining adversarial finding

- Iteration 2 retained pending agent IDs in GET but the matcher did not consult them after a skill was loaded later.

### Correction and observed result

- Added a legacy agent fallback below conversation and explicit durable agent records and above global state.
- Added `agent_binding_set_before_load_filters_future_skills`; it selected the pending later-loaded skill, excluded another later-loaded skill, and observed conversation scope override the fallback.
- Observed service matrix `7 passed; 0 failed`, existing agent API `2 passed; 0 failed`, and both B4 integration assertions `2 passed; 0 failed`.
- Replayed the live-effect inversion through the single domain precedence path: exit 101; restored domain SHA-256 `f376d394f5188f372585b8d2ad7dd0c61b3d069b25f337076e0b768a009dd2e1`; positive exit 0.
- Tier 0 check and Clippy exited 0; Tier 2 remains deferred.
