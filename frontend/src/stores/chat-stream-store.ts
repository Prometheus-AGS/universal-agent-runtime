...
import { postChatCompletion, CHAT_COMPLETION_URL } from "@/services/chat-stream-api";
...
let retryPolicyCache: { value: StreamRetryPolicy; loadedAt: number } | null = null;
...
function retryPolicyFromSettingsResponse(settings: unknown): StreamRetryPolicy {
  ...
  return {
    retriesEnabled: toBoolean(read("retries_enabled"), DEFAULT_STREAM_RETRY_POLICY.retriesEnabled),
    maxAttempts: Math.max(1, Math.trunc(toNumber(read("retry_max_attempts"), DEFAULT_STREAM_RETRY_POLICY.maxAttempts))),
    streamStartTimeoutMs: Math.max(1000, Math.trunc(toNumber(read("stream_start_timeout_ms"), DEFAULT_STREAM_RETRY_POLICY.streamStartTimeoutMs))),
    baseDelayMs: Math.max(100, Math.trunc(toNumber(read("retry_base_delay_ms"), DEFAULT_STREAM_RETRY_POLICY.baseDelayMs))),
    backoffMultiplier: Math.max(1.1, toNumber(read("retry_backoff_multiplier"), DEFAULT_STREAM_RETRY_POLICY.backoffMultiplier)),
    maxDelayMs: Math.max(100, Math.trunc(toNumber(read("retry_max_delay_ms"), DEFAULT_STREAM_RETRY_POLICY.maxDelayMs))),
    respectRetryAfter: toBoolean(read("retry_respect_retry_after"), DEFAULT_STREAM_RETRY_POLICY.respectRetryAfter),
    retryableHttpStatuses: toNumberArray(read("retryable_http_statuses"), DEFAULT_STREAM_RETRY_POLICY.retryableHttpStatuses),
    retryableTransportErrors: toBoolean(read("retryable_transport_errors"), DEFAULT_STREAM_RETRY_POLICY.retryableTransportErrors),
    retryBudgetMs: Math.max(0, Math.trunc(toNumber(read("retry_budget_ms"), DEFAULT_STREAM_RETRY_POLICY.retryBudgetMs))),
  };
}

async function loadStreamRetryPolicy(): Promise<StreamRetryPolicy> {
  const now = Date.now();
  if (retryPolicyCache && now - retryPolicyCache.loadedAt <= RETRY_POLICY_CACHE_TTL_MS) {
...
class HttpStatusError extends Error {
  ...
  constructor(status: number, statusText: string, responseBody: string, retryAfterHeader: string | null) {
...
function parseRetryAfterMs(retryAfterHeader: string | null, maxDelayMs: number): number | null {
...
function shouldRetryStreamAttempt(
  ...
  if (sawFirstStreamChunk) return { retry: false, delayMs: 0, reason: "stream_already_started" };
  if (!policy.retriesEnabled) return { retry: false, delayMs: 0, reason: "retries_disabled" };
  if (attempt + 1 >= policy.maxAttempts) return { retry: false, delayMs: 0, reason: "max_attempts_reached" };
  if (policy.retryBudgetMs <= 0) return { retry: false, delayMs: 0, reason: "retry_budget_exceeded" };

  const budgetAllows = (delayMs: number): boolean => cumulativeRetryDelayMs + delayMs <= policy.retryBudgetMs;
  if (error instanceof HttpStatusError) {
    if (!isRetryableHttpStatus(error.status, policy)) return { retry: false, delayMs: 0, reason: "http_status_not_retryable" };
    const delayMs = computeRetryDelayMs(attempt, error.retryAfterHeader, policy);
    if (!budgetAllows(delayMs)) return { retry: false, delayMs: 0, reason: "retry_budget_exceeded" };
...
interface ChatStreamActions {
  startStream: (threadId: string, payload: UarChatPayload, callbacks?: StreamCallbacks) => Promise<void>;
  cancelStream: () => void;
}
...
export const useChatStreamStore = create<ChatStreamActions>(() => ({
  ...
  startStream: async (threadId: string, payload: UarChatPayload, callbacks?: StreamCallbacks): Promise<void> => {
    useChatStreamStore.getState().cancelStream();
      ...
      for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
        ...
        try {
          const res = await postChatCompletion(requestBody, threadId, controller.signal);

          if (!res.ok) {
            const body = await res.text().catch(() => "(no response body)");
            throw new HttpStatusError(res.status, res.statusText, body, res.headers.get("retry-after"));
          }
          ...
          const normalizedError = streamStartTimedOut
            ? Object.assign(new Error(`Stream start timeout after ${retryPolicy.streamStartTimeoutMs}ms`), { name: "StreamStartTimeoutError" })
            : err;
          ...
          if (retryDecision.retry) {
            ...
            useChatMessageStore
              .getState()
              .setAwaitingRetry(threadId, runId, attempt + 1, maxAttempts, retryDecision.delayMs);
          ...
          const error = normalizedError instanceof Error ? normalizedError : new Error(String(normalizedError));
          const ts = new Date().toISOString();
          const detail = error instanceof HttpStatusError
