...
const chartConfig: ChartConfig = {
  cost: { label: "spend (USD)", color: "hsl(var(--phosphor))" },
};
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden font-mono text-[13px] text-[hsl(var(--terminal-fg))]">
      ...
      <div className="flex items-center justify-between border-b border-[hsl(var(--terminal-line-strong))] bg-[hsl(var(--terminal-surface))] px-6 py-4">
        <div>
          <h2 className="text-[20px] font-medium tracking-tight text-[hsl(var(--terminal-fg))]">cost</h2>
      ...
      <div className="flex-1 overflow-y-auto">
        {/* Stat tiles */}
        <section
          className="grid grid-cols-2 divide-x divide-[hsl(var(--terminal-line))] border-b border-[hsl(var(--terminal-line))] md:grid-cols-4"
        ...
        <section className="border-b border-[hsl(var(--terminal-line))] px-6 py-4" aria-label="Spend over time">
          ...
          {chartData.length === 0 ? (
            ...
            <ChartContainer config={chartConfig} className="aspect-auto h-56 w-full">
              <BarChart data={chartData} accessibilityLayer>
                <CartesianGrid vertical={false} stroke="hsl(var(--terminal-line))" />
                <XAxis
                  dataKey="index"
                  tickLine={false}
                  axisLine={false}
                  tickMargin={8}
                  stroke="hsl(var(--terminal-fg-dim))"
                  fontSize={11}
                />
                <YAxis
                  tickLine={false}
                ...
                <Bar dataKey="cost" fill="hsl(var(--phosphor))" radius={2} />
              </BarChart>
            </ChartContainer>
          )}
        </section>
        ...
        {/* Per-model breakdown */}
        <section className="border-b border-[hsl(var(--terminal-line))] px-6 py-4" aria-label="Spend by model">
          ...
          {byModel.length === 0 ? (
            ...
            <div className="flex flex-col gap-1.5" data-testid="cost-by-model">
              {byModel.map((row) => (
                <div
                  key={row.model}
                  className="flex items-center justify-between gap-3 border border-[hsl(var(--terminal-line-strong))] bg-[hsl(var(--terminal-surface))] px-4 py-2.5"
                >
                  <span className="truncate text-[13px] text-[hsl(var(--terminal-fg))]">{row.model}</span>
                  <span className="shrink-0 font-medium text-[hsl(var(--phosphor))]">${row.spend.toFixed(4)}</span>
                </div>
              ))}
            </div>
          )}
  ...
  return (
    <div className="px-4 py-3">
      <div className="mb-2 flex items-center justify-between">
        <span className="text-xs text-[hsl(var(--terminal-fg-dim))]">{label}</span>
        <Icon
          size={15}
          className={cn(
            tone === "amber" && "text-[hsl(var(--amber))]",
            tone === "red" && "text-[hsl(var(--signal-red))]",
            !tone && "text-[hsl(var(--phosphor))]",
          )}
        />
      </div>
      <div
  ...
  return (
    <div
      className={cn(
        ...
        alert.exceeded
          ? "border-[hsl(var(--signal-red)/0.5)] bg-[hsl(var(--signal-red)/0.06)]"
          : "border-[hsl(var(--amber)/0.5)] bg-[hsl(var(--amber)/0.06)]",
      )}
      data-testid={`budget-alert-${alert.id}`}
    >
      <AlertTriangle
        size={14}
        className={cn("shrink-0", alert.exceeded ? "text-[hsl(var(--signal-red))]" : "text-[hsl(var(--amber))]")}
      />
      <div className="min-w-0 flex-1">
        <p className="truncate text-[13px] text-[hsl(var(--terminal-fg))]">
      ...
      <span
        className={cn(
          ...
          alert.exceeded
            ? "border-[hsl(var(--signal-red)/0.5)] text-[hsl(var(--signal-red))]"
