...
import { CheckCircle2, ChevronRight, Circle, Loader2, Plus, RefreshCw, Server, XCircle } from "lucide-react";
...
export const ProvidersPage: FC = () => {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm(`Remove configuration for provider "${id}"?`)) return;
    try {
      await fetch(`/api/uar/providers/${id}`, { method: "DELETE" });
      ...
      <div className="flex w-72 shrink-0 flex-col border-r border-border bg-background">
        <div className="flex items-center justify-between border-b border-border px-4 py-3">
          <div>
            ...
            <p className="font-mono text-[10px] text-muted-foreground">{catalog.length} available · {configured.length} configured</p>
          </div>
        ...
        <div className="flex gap-1 border-b border-border px-3 py-2">
          {(["all", "configured", "unconfigured"] as const).map((f) => (
            <button
              ...
              onClick={() => setFilter(f)}
              className={cn(
                "rounded px-2 py-0.5 font-mono text-[10px] transition-colors capitalize",
        ...
        <div className="flex-1 overflow-y-auto py-2">
          ...
          {!loading && visible.length === 0 && (
            <p className="px-4 py-4 font-mono text-[11px] text-muted-foreground">No providers found</p>
          )}
          {visible.map((p) => (
            <Button
              key={p.id}
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate font-display text-[13px] font-semibold text-foreground">{p.display_name}</p>
                <p className="font-mono text-[10px] text-muted-foreground">{p.model_count} models</p>
              </div>
              {p.id === defaultId && (
        ...
        <DialogContent>
          ...
          <div className="flex flex-col gap-3">
            {addTarget?.auth_env_var && (
              <p className="rounded-md border border-border bg-muted/40 px-3 py-2 font-mono text-[11px] text-muted-foreground">
                Tip: set <code className="text-foreground">{addTarget.auth_env_var}</code> env var instead of storing a key here.
              </p>
            )}
            <div>
              <Label htmlFor="cfg-api-key" className="mb-1 block font-mono text-[11px] text-muted-foreground">API Key</Label>
              <Input
                id="cfg-api-key"
                type="password"
                value={form.api_key}
            ...
            <div>
              <Label htmlFor="cfg-base-url" className="mb-1 block font-mono text-[11px] text-muted-foreground">Base URL Override</Label>
              <Input
                id="cfg-base-url"
                value={form.base_url}
...
function ProviderDetail({ provider, configured, isDefault, onSetDefault, onConfigure, onDelete }: ProviderDetailProps) {
    ...
    <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-2">
              ...
              {provider.configured
                ? <Badge variant="outline" className="gap-1 border-success/40 text-success text-[10px]"><CheckCircle2 size={10} />Configured</Badge>
                : <Badge variant="outline" className="gap-1 text-muted-foreground text-[10px]"><XCircle size={10} />Not Configured</Badge>
              }
              {isDefault && <Badge className="text-[10px]">Default</Badge>}
            </div>
            <p className="mt-0.5 font-mono text-[11px] text-muted-foreground">{provider.base_url ?? "Provider-specific URL"}</p>
        ...
        {provider.endpoints.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-1">
            {provider.endpoints.map((ep) => (
              <span key={ep} className="rounded border border-border px-1.5 py-0.5 font-mono text-[9px] text-muted-foreground">{ep}</span>
            ))}
          </div>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4">
        <div className="mb-3 flex items-center gap-2">
          <p className="font-mono text-[11px] font-medium uppercase tracking-widest text-muted-foreground">Models ({provider.model_count})</p>
        ...
        {loadingModels ? (
          ...
          <p className="font-mono text-[11px] text-muted-foreground">Model details will appear once this provider is configured.</p>
        ) : (
          <div className="flex flex-col gap-1">
            {models.map((m) => (
              <div key={m.id} className="flex items-center gap-3 rounded-md border border-border/50 bg-card px-3 py-2.5">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate font-mono text-[12px] text-foreground">{m.name || m.id}</p>
                  <p className="font-mono text-[10px] text-muted-foreground">{m.id}</p>
                </div>
                <div className="flex shrink-0 items-center gap-1">
                  {m.context > 0 && <span className="font-mono text-[10px] text-muted-foreground">{(m.context / 1000).toFixed(0)}k</span>}
                  {m.tool_call && <span className="rounded border border-border px-1 font-mono text-[9px] text-muted-foreground">tools</span>}
                  {m.reasoning && <span className="rounded border border-border px-1 font-mono text-[9px] text-muted-foreground">reasoning</span>}
                </div>
              </div>
            ))}
          </div>
