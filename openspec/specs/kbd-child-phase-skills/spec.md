# kbd-child-phase-skills Specification

## Purpose

Two skills (`/kbd-new-child`, `/kbd-next-child`) that create and traverse child phases owned by the currently-active top-level phase. Honour `parentPhase` / `childPhases` / `childPointer` semantics from `kbd-nested-phase-schema` and fire `child:before` / `child:after` per `kbd-process-hooks`. Single-level nesting only.
