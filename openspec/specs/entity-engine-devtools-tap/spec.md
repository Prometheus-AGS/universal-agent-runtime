# entity-engine-devtools-tap Specification

## Purpose

In-process push stream of mutating graph events from `prometheus-entity-management`, consumed by dev tooling (Entity Explorer's Events tab; future buffer / multi-store registry). `subscribeDevtoolsEvent(cb): UnsubscribeFn` returns a no-op cleanup function from `src/engine.ts`; `notifyDevtools(event)` is fired by every mutating op in `src/graph-actions.ts` (`upsertEntity`, `patchEntity`, `clearPatch`). Hot-path is a no-op when no subscribers are registered; op-site calls are guarded with `process.env.NODE_ENV !== "production"` so consumer bundlers can elide them from production builds. The `DevtoolsEvent` discriminated union is forward-compatible with future `unpatch` and `list` kinds.
