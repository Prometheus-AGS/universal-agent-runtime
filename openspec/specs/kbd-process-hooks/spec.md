# kbd-process-hooks Specification

## Purpose

Extensible hook surface fired around every KBD lifecycle boundary — phases, child phases, plans, executes, reflects, individual tasks, and assessments. Hooks support augment-vs-override modes with deterministic project > user > builtin layer precedence, a uniform `KBD_HOOK_*` context payload, a built-in `report-progress` reporter emitting `starting/ending <kind> <name> [<i>/<n>]` to stderr, and an append-only JSONL audit log under each phase directory.
