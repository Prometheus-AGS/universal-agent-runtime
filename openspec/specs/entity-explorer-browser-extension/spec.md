# entity-explorer-browser-extension Specification

## Purpose

A Chrome MV3 extension that observes any page importing `@prometheus-ags/prometheus-entity-management`, communicates via a `__PROMETHEUS_DEVTOOLS_HOOK__` page-side global + `postMessage` bridge with content-script relay, and renders the same five-tab Entity Explorer panel from `entity-explorer-fab-panel` (change 10) inside Chrome DevTools. Reuses the host package's panel components through a shared `DevtoolsDataSource` abstraction. Stretch / v2 scaffold — manifest + bridge contract + architecture notes; implementation deferred.
