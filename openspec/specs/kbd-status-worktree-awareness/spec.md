# kbd-status-worktree-awareness Specification

## Purpose

`/kbd-status` renders the active worktree's top-level path on every invocation, warns when the checkout is not a descendant of the configured `worktreeRoot`, and renders the active phase as a parent → child chain when the nested-phase fields are populated. Every new piece of information degrades gracefully: a missing `git`, unreadable `project.json`, or pre-schema waypoint never blocks the status report.
