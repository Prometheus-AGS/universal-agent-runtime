# kbd-new-phase-skill Specification

## Purpose

A first-class `/kbd-new-phase <name> [goals…]` command that manually creates a fresh top-level KBD phase, seeds `goals.md` and `progress.json` (with nested-phase defaults), flips `current-waypoint.json` and `project.json` atomically, fires `phase:before`, and emits canonical Progress Signals. Manual-entry counterpart to `/kbd-next-phase`.
