# uar-worktree-convention Specification

## Purpose

Worktrees for the `universal-agent-runtime` repository live under `~/.claude/worktrees/`, never inside the repository working tree, and are managed through repo-shipped helper scripts. This avoids namespace collision with the checked-in `.claude/` tool configuration consumed by Roo, Cursor, Codex, OpenCode, and Claude Code.
...
This convention SHALL NOT relocate, rename, or otherwise touch any worktree that exists at the time of adoption.

#### Scenario: Existing in-repo worktree at adoption time
- **WHEN** the convention is adopted while `.claude/worktrees/adoring-booth-312094` (or any other in-repo worktree) exists
- **THEN** the adoption MUST leave that worktree's files and `git worktree` registration untouched; the convention applies only to worktrees created after adoption.
