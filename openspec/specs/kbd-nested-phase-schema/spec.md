# kbd-nested-phase-schema Specification

## Purpose

Extends the KBD orchestrator's `current-waypoint.json` and `project.json` schemas with optional, additive fields that support nested (parent → child) phases and codify the worktree-root convention introduced by `uar-worktree-convention`. All fields are backward-compatible: tools that predate this schema continue to work unchanged.
