...
- Per-step projection: each `ResolvedStep` names the exact servers and tools; a lower-authority skill cannot replace a definition or weaken sandbox or network restrictions. Local child agents retain narrowed frozen bindings, while remote UAR peers resolve their own verified root catalog.
...
- Global servers keep eager startup; skill-contributed servers start lazily on first call when their cached catalog is complete, with `wait_until_ready` on the call path. Delegation never transfers a connection recipe: local children retain frozen bindings and remote UAR peers start from their own root catalog.
