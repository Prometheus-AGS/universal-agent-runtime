...
- Rewrite `frontend/src/components/ui/command.tsx` using Base UI native `Autocomplete`:
  ```tsx
  import { Autocomplete } from '@base-ui/react/autocomplete';
  ```
- `Autocomplete` supersedes the proposal's original `Combobox` detail because these
  wrappers filter action lists without persisting a selected form value; this follows
  the current Base UI usage guidance and preserves repeated multi-add flows.
...
- `frontend-component-primitives`: require Base UI ownership for application command-search wrappers and reproducible documentation of accepted third-party Radix transitives.
