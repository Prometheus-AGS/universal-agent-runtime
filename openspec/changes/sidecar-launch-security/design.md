...
- `start_server_sidecar` (`src/server.rs:1998-2016`) is called only by the sidecar binary. The live test harness uses `start_server_sidecar_with_listeners_and_shutdowns` (`tests/integration/live/harness.rs:24,564`) and must keep working. *Amended 2026-09-23 after the adversarial diff review:* that function and `start_server_sidecar_with_listeners` install no launch-token guard, so they are renamed `start_server_with_listeners_and_shutdowns` and `start_server_with_listeners`, with doc comments stating they are not sidecar entry points; `start_server_sidecar` is the only sidecar-named start function and it always requires a `SidecarLaunchToken`.
