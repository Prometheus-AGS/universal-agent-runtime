...
- [x] 2.15 `standalone_server_keeps_companion_grpc_and_spa` — standalone start (criterion amended 2026-09-23: the implemented test runs the real `universal-agent-runtime` binary as a process instead of an in-process `start_server_with_listener`-backed entry; a real process exercises the production boot path and is the stronger check, so the in-process form is not also required — same amendment for 2.17, 2.18, 2.30 and 2.31); asserts companion bind attempt, gRPC listener and SPA index still served. *Regression guard.*
...
- [x] 4.3 Run the existing governance, middleware and live-harness tests once to confirm no regression; record output. — 2026-09-23 (`evidence/review-fix-batch.txt`): `cargo test --lib -- governance::runtime_control security::middleware` 24 passed / 0 failed; `cargo test --features test-probes --test integration -- live::harness` 1 + 4 passed (also confirms the entry-point rename); earlier gate: `settings_persistence` 47/47 (`gate-run-1.txt`). Same batch verified 1.4: `cargo build --bin uar-env-probe --features test-probes` succeeds and the build without the feature is refused (`requires the features: test-probes`, exit 101).
