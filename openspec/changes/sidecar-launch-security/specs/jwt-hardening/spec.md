...
The runtime SHALL classify governance as operator-optional only after boot proves all four conditions: the boot-effective configured `server.host` literal is exactly `localhost` or `127.0.0.1`; the authentication middleware installed for the process does not require JWT; the process is not a supervised sidecar authenticated by a launch token; and every declared tool-capable ingress has supplied a successfully bound loopback address to a sealed inventory. No other configured host spelling or address SHALL qualify. Requests without credentials SHALL continue with the anonymous principal in that posture.
...
#### Scenario: Token-authenticated sidecar keeps governance mandatory
- **WHEN** the supervised sidecar boots with a launch token, JWT not required, `server.host` of `127.0.0.1`, and only a successfully bound loopback primary ingress
- **THEN** governance remains mandatory with reason `host_token_required`, and an attempt to save `governance.enabled` as `false` is rejected
