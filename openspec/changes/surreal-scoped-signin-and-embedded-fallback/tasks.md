...
- [ ] 4.6 Sidecar global-MCP lock obligation (carried from `sidecar-launch-security` design, "Known limitation"): the lock is read from the settings manager, which exists only when persistence exists. If this change can leave a sidecar without persistence or without a settings manager in any path (remote unreachable, fallback failure, `remote_fallback: none`), keep the settings manager present in sidecar mode or move the lock to an explicit sidecar-mode flag, and add a contract test that global MCP `PUT`/`DELETE /api/uar/mcp/servers/x` still return 409 on the embedded-fallback path.
