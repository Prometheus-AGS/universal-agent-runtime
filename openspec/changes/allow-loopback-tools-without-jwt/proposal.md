...
- Dependencies and frontend: no dependency is added. The hand-authored Governance panel, settings API, typed service/store hooks, and normalized entity projection change narrowly so the UI can distinguish authoritative On, Off, Required, Unknown, mutation-unavailable, pending-save, partial, rejected, and changed-elsewhere states instead of rendering a schema-only boolean.
