...
- [ ] 1.1 In `src/embedded.rs` (near the existing `SkillService` construction at
...
- [ ] 1.2 Ensure ordering with `initialize()` does not double-register. Prefer
      idempotent registration over ordering assumptions — assert it in 2.3.

## 2. Proof

- [ ] 2.1 Test: fresh embedded database → built-ins present in the registry.
- [ ] 2.2 Test: restart against the same database → built-ins still present and
      loaded from persistence.
- [ ] 2.3 Test: two starts → each built-in appears exactly once.
- [ ] 2.4 Test: `seed_defaults` disabled → no built-ins seeded.
- [ ] 2.5 **Negative control.** Remove the registration call in a scratch build
      and show 2.1 fails. Record the command and its failing output.

## 3. Spec correction

- [ ] 3.1 Amend `docs/SPECIFICATION.md:445`: correct the call sites
      (`server.rs:448` and `:511`, not `:436`), and replace "empty skill
