...
distributed server/sidecar product. Each tool is its own Cargo workspace
member — build and install it independently of the main
...
See also [Troubleshooting → HTTP 401 Unauthorized on API calls](../troubleshooting.md#http-401-unauthorized-on-api-calls) for the underlying JWT requirement this tool works around, and [Configuration → Security](../configuration/intro.md) for `security.jwt_secret`/`jwt_required`.
