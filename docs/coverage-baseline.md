...
Both are enforced in [`.github/workflows/coverage.yml`](../.github/workflows/coverage.yml)
with a **60% line-coverage floor** (`--fail-under-lines 60` / vitest
...
<!-- PENDING: task 3.1 (openspec/changes/coverage-cargo-llvm-cov-60pct/tasks.md)
     records the actual measured percentages here after the first coverage
     workflow run on `main`. Per the KBD implementation-first policy, that
...
| Path | Line coverage |
|---|---|
| _(populate from the first `coverage.yml` run on `main`)_ | _TBD_ |
...
- Update this file whenever the baseline intentionally moves (e.g. after
  ...
  60%).
- The threshold itself (currently 60%) lives in `.github/workflows/coverage.yml`
  (`--fail-under-lines 60`) and `frontend/vitest.config.ts` (`coverage.thresholds`).
