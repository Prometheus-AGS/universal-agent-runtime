...
| `/admin/runtime` cockpit | `admin/pages/runtime-console-page` | inspect live runs, provider health and activity; `/api/uar/providers/health` plus normalized runtime events | `runtime-console-governance-certification`; `runtime-console-visual.spec.ts`, `runtime-event-replay.spec.ts` | Certified stable | Complete (`certify-runtime-console-governance`) |
| `/admin/runs` | runtime console domain | inspect/replay/cancel runs; run detail/stream/cancel contracts | `runtime-console-governance-certification`; replay E2E | Certified stable | Complete (`certify-runtime-console-governance`) |
| `/admin/approvals` | runtime console domain | approve/deny tool calls; `POST /api/uar/runs/{id}/tool-approval` | tri-state governance unit/store/browser acceptance | Certified stable | Complete (`certify-runtime-console-governance`) |
...
| `/admin/credentials` | credential store/service | list/upsert/delete masked credentials; `/api/uar/credentials[/{provider}]` | no focused OpenSpec baseline; `credentials-page.test.ts` is unit-only | Stable candidate | `certify-remaining-admin-surfaces` |
...
| `/admin/skills` | skill entity domain | list/create/update/toggle/import; `/api/skills[/{id}[/toggle]]`, `/api/uar/skills/import` | `skill-hot-reload`; `admin-skills.spec.ts`, utility unit tests | Stable candidate | `certify-remaining-admin-surfaces` |
| `/admin/agents` | agent entity domain | list/create/update/delete/status; `/api/agents[/{id}]` | `agent-status-ui`; `admin-agents.spec.ts`, agent selection E2E | Stable candidate | `certify-remaining-admin-surfaces` |
| `/admin/tools` | tool entity domain | discover/inspect tools; `/api/tools`; execution only through governed runtime | `tool-approval-workflow` covers execution governance; `admin-tools.spec.ts` | Stable candidate (discovery) | `certify-remaining-admin-surfaces` |
| `/admin/auth` | auth store/service | list/create/revoke API keys; `/api/auth/keys[/{id}]` | `auth-key-management`; comprehensive backend tests, browser acceptance missing | Stable candidate | `certify-remaining-admin-surfaces` |
...
| `/admin/memory` | memory entity domain | list/stats/delete/clear; `/api/admin/memories...` | no focused product-surface spec; browser acceptance missing | Stable when enabled | `certify-remaining-admin-surfaces` |
| `/admin/compiler` | compiler domain | list/create compiler sessions; `/api/compiler/sessions` | no complete compile/package lifecycle certification | Experimental (visibly labelled) | `certify-remaining-admin-surfaces` |
...
| `/admin/mcp-health` | MCP domain | inspect server health; `/api/uar/mcp/health` | `mcp-health-dashboard`; browser acceptance missing | Stable candidate diagnostic | `certify-remaining-admin-surfaces` |
| `/admin/cost` | runtime cost entities | inspect usage/budget alerts; runtime event/entity contracts | no focused product-surface spec; browser acceptance missing | Stable candidate read-only | `certify-remaining-admin-surfaces` |
